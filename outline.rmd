...
library(stargazer)
data %>% 
  select(pop, mig, migratio, y, AGE, INCWAGE, unmp_rate, campclosest_dist) %>%
  as.data.frame() %>% 
  stargazer(
    # type = "text", 
    ...
    covariate.labels = c(
      ...
      "Japanese/Total Migration Ratio", "Average Age", "Average Wage",
      "Unemployment Rate", "Distance to Closest Camp"
    ),
    digits = 2
  )
